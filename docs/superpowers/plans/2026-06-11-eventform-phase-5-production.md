...
- [ ] **Step 3: ComputeStack** — Vpc (maxAzs 1, natGateways 0, public subnet), SG (22/80/443 from anywhere — comment: tighten 22 to your IP), t3.small AL2023 instance with userData (dnf install docker, enable, usermod ec2-user, install compose plugin, mkdir /opt/eventform), EIP association, optional Route53 (context `hostedZoneId`/`zoneName` → two A records) else CfnOutput of the EIP. Output instance id + EIP.

- [ ] **Step 4: Assertion tests** (the verification — no AWS): AuthStack template has `AWS::Cognito::UserPool` with AdminCreateUser config, IdP Google with correct scopes, client with `AllowedOAuthFlows: [code]` + correct callbacks; ComputeStack has t3.small, SG with the 3 ingress rules, EIP, and (with zone context) two A records. ~8 tests.

- [ ] **Step 5:** `pnpm --filter eventform-cdk test` green; `pnpm --filter eventform-cdk exec cdk synth -c googleClientId=x -c googleClientSecret=y --all` produces both templates. (cdklocal note: Community LocalStack can't emulate Cognito or real EC2 — synth + assertions ARE the local verification; record this in the plan notes.)

- [ ] **Step 6: Commit** — `feat(infra): cdk stacks for cognito auth and ec2 compute`
...
- [ ] **Step 2: docs/DEPLOYMENT.md** — the handoff checklist, numbered, copy-pasteable: (1) AWS account + `cdk bootstrap`; (2) Google Cloud Console OAuth client (exact console path, authorized redirect URI `https://<cognito-domain>.auth.<region>.amazoncognito.com/oauth2/idpresponse`); (3) `cdk deploy AuthStack -c googleClientId=… -c googleClientSecret=…` → note outputs; (4) `cdk deploy ComputeStack` (+ Route53 context or manual A records to the EIP); (5) EC2 first-time setup: clone repo to /opt/eventform, write `.env` (every `${VAR}` from the prod compose with explanations), run `infra/prod/bootstrap.sh`, `docker compose -f docker-compose.prod.yml up -d`, register connector (connect-init does it), run migrations; (6) GitHub repo secrets/variables for deploy.yml; (7) smoke checklist (https loads, Google login works, full loop, failed-delivery demo script for recruiters); (8) teardown/cost table.
